...
static constexpr byte KOREAN_FONT_PREFIX_FIRST = 0xC0;
static constexpr byte KOREAN_FONT_PREFIX_LAST = 0xCC;
static constexpr byte KOREAN_FONT_PREFIX_C0 = KOREAN_FONT_PREFIX_FIRST;
static constexpr int KOREAN_FONT_PAGE_COUNT = KOREAN_FONT_PREFIX_LAST - KOREAN_FONT_PREFIX_FIRST + 1;
static constexpr int KOREAN_FONT_WIDTH = 31;
static constexpr int KOREAN_FONT_LEFT_PADDING = 0;

static bool is_korean_font_prefix(byte value)
{
  return value >= KOREAN_FONT_PREFIX_FIRST && value <= KOREAN_FONT_PREFIX_LAST;
}

static int get_korean_font_page(byte prefix)
{
  return prefix - KOREAN_FONT_PREFIX_FIRST;
}

static void get_korean_char_metrics(int& charWidth, int& leftPadding)
{
  charWidth = KOREAN_FONT_WIDTH;
  leftPadding = KOREAN_FONT_LEFT_PADDING;
}

static ff7_graphics_object* get_korean_font_graphics_object(byte prefix)
{
  if (is_korean_font_prefix(prefix))
  {
    const int page = get_korean_font_page(prefix);
    if (ff7_externals.menu_hangulfont_graphics_objects[page])
      return ff7_externals.menu_hangulfont_graphics_objects[page];
  }

  return ff7_externals.menu_jafont_1_graphics_object;
}

static void draw_korean_font_graphics_objects(ff7_game_obj* game_object)
{
  for (int page = 0; page < KOREAN_FONT_PAGE_COUNT; ++page)
  {
    if (ff7_externals.menu_hangulfont_graphics_objects[page])
      ff7_externals.engine_draw_graphics_object_66E641(ff7_externals.menu_hangulfont_graphics_objects[page], game_object);
  }
}

static void reset_korean_font_graphics_objects()
{
  for (int page = 0; page < KOREAN_FONT_PAGE_COUNT; ++page)
  {
    if (ff7_externals.menu_hangulfont_graphics_objects[page])
      ff7_externals.reset_field_54_graphics_object_66E62C(ff7_externals.menu_hangulfont_graphics_objects[page]);
  }
}
...
void engine_load_menu_graphics_objects_6C1468_jp(int a1)
{
  ...
  if ( viewport_type_404D80 == 2 )
  {
    ...
    for (int page = 0; page < KOREAN_FONT_PAGE_COUNT; ++page)
      ff7_externals.sub_671082(&ff7_externals.menu_hangulfont_graphics_objects[page]);
  }
  else
  {
  ...
  if ( viewport_type_404D80 == 2 )
  {
    ...
    for (int page = 0; page < KOREAN_FONT_PAGE_COUNT; ++page)
    {
      char hangul_font_path[32];
      _snprintf(hangul_font_path, sizeof(hangul_font_path), "hangulfont_%d.tim", page + 1);
      ff7_externals.engine_set_blendmode_674659(4, &a2);
      ff7_externals.menu_hangulfont_graphics_objects[page] = ff7_externals.engine_load_graphics_object_6710AC(
                                             1,
                                             12,
                                             &a2,
                                             hangul_font_path,
      ...
      if (!ff7_externals.menu_hangulfont_graphics_objects[page])
      {
        _snprintf(hangul_font_path, sizeof(hangul_font_path), "hangulfont_%d.tex", page + 1);
        ff7_externals.engine_set_blendmode_674659(4, &a2);
        ff7_externals.menu_hangulfont_graphics_objects[page] = ff7_externals.engine_load_graphics_object_6710AC(
                                               1,
                                               12,
        ...
        float z_value)
{
        ...
        ++i )
  {
    ...
    {
      if (is_korean_font_prefix(*buffer_text))
      {
        ++(*ff7_externals.field_text_box_curr_n_characters_DC3CB0);
        byte korean_font_prefix = *buffer_text;
        ++buffer_text;
        graphics_object = get_korean_font_graphics_object(korean_font_prefix);
        kanjiDetected = true;
        get_korean_char_metrics(charWidth, leftPadding);
        continue;
      }

      switch ( *buffer_text )
      {
        case KOREAN_FONT_PREFIX_C0:
          ++(*ff7_externals.field_text_box_curr_n_characters_DC3CB0);
          ++buffer_text;
          graphics_object = get_korean_font_graphics_object(*buffer_text);
          kanjiDetected = true;
          get_korean_char_metrics(charWidth, leftPadding);
          continue;
        case 0xFAu:
...
void field_draw_text_boxes_and_text_graphics_object_6ECA68_jp()
{
  ...
  else
  {
    ...
    if ( *ff7_externals.field_do_draw_character_DC3CEC )
    {
      ...
      draw_korean_font_graphics_objects(game_object);
      ...
      reset_korean_font_graphics_objects();
...
int common_submit_draw_char_from_buffer_6F564E_jp(int x, int vertex_y, int n_shapes, unsigned __int16 letter, float z_value)
{
  ...
  if (is_korean_font_prefix((byte)letter))
  {
    p_letter = (unsigned __int16 *)((byte *)&letter + 1);
    character_graphics_object = get_korean_font_graphics_object((byte)letter);
    get_korean_char_metrics(charWidth, leftPadding);
    goto LABEL_9;
  }
...
void draw_text_top_display_6D1CC0_jp(int a1, __int16 menu_box_idx, char a3, unsigned __int16 a4)
{
  ...
  else
  {
    ...
    for ( j = 0; j < 256 && text_sub_41963C->name[0] != 255; ++j )
    {
      ...
      if (is_korean_font_prefix(text_sub_41963C->name[0]))
      {
    ...
    {      
      if (is_korean_font_prefix(v135->name[0]))
      {
        byte korean_font_prefix = v135->name[0];
      ...
      switch ( v135->name[0] )
      {
        ...
        case KOREAN_FONT_PREFIX_C0:
          v135 = (attack_name_fixed_buffer *)((char *)v135 + 1);
          graphics_object = get_korean_font_graphics_object(v135->name[0]);
          get_korean_char_metrics(charWidth, leftPadding);
          isKanjiDetected = true;
          continue;
        case 0xFAu:
...
void auto_resize_text_box(int16_t WINDOW_ID, int16_t* pOutW, int16_t* pOutH)
{
  ...
  int leftPadding = 0;
    ...
    if (is_korean_font_prefix(character))
    {
      get_korean_char_metrics(charWidth, leftPadding);
      isKanjiDetected = true;
      continue;
